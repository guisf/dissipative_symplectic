    ...
    def fit(self, X, y=None):
        ...
        for _ in self.progressbar(range(self.n_iterations)):
            ...
            for batch in batch_iterator(X, batch_size=self.batch_size):
                ...
                self.W = self.W + self.lr*self.mu*self.pW
                self.h0 = self.h0 + self.lr*self.mu*self.ph0
                self.v0 = self.v0 + self.lr*self.mu*self.pv0
                ...
                self.pW = self.mu*self.pW - self.lr*g1
                self.ph0 = self.mu*self.ph0 - self.lr*g2
                self.pv0 = self.mu*self.pv0 - self.lr*g3

                self.W = oldW + self.lr*self.pW
                self.h0 = oldh0 + self.lr*self.ph0
                self.v0 = oldv0 + self.lr*self.pv0
...
if __name__ == '__main__':
    ...
    train_temps = [temps[i] for i in range(0, len(temps), config.nest_skip)]
    N = config.N
    for i, T in enumerate(train_temps):
        ...
        Z[Z==-1] = 0
        idx = np.random.choice(range(Z.shape[0]), config.nest_num_train_points,
                               replace=False)
        X = Z[idx]

        # training RBM
        rbm = RBM(n_hidden=config.nest_n_hidden,
                  batch_size=config.nest_batch_size,
                  learning_rate=config.nest_learning_rate,
                  mu=config.nest_mu,
                  n_iterations=config.nest_iterations)
        rbm.fit(X)
        pickle.dump(rbm.training_errors,
                    open(config.nest_conv_format%(T,N), 'wb'))
        ...
        pickle.dump(states, open(config.nest_file_format%(T, N), 'wb'))
