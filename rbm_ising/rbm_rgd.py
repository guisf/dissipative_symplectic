...
class RBM():
    ...
    def __init__(self, n_hidden=128, learning_rate=0.1, mu=0.9, c=0,
    ...
    def fit(self, X, y=None):
        ...
        for _ in self.progressbar(range(self.n_iterations)):
            ...
            for batch in batch_iterator(X, batch_size=self.batch_size):
                ...
                self.W = self.W + (h*c*mu/2.)*self.pW/np.sqrt((mu**2)*(np.linalg.norm(self.pW)**2)+c**2)
                self.h0 = self.h0 + (h*c*mu/2.)*self.ph0/np.sqrt((mu**2)*(np.linalg.norm(self.ph0)**2)+c**2)
                self.v0 = self.v0 + (h*c*mu/2.)*self.pv0/np.sqrt((mu**2)*(np.linalg.norm(self.pv0)**2)+c**2)
                ...
                self.pW = mu*self.pW - h*g1
                self.ph0 = mu*self.ph0 - h*g2
                self.pv0 = mu*self.pv0 - h*g3
                
                self.W = self.W + (h*c/2.)*self.pW/np.sqrt((np.linalg.norm(self.pW)**2)+c**2)
                self.h0 = self.h0 + (h*c/2.)*self.ph0/np.sqrt((np.linalg.norm(self.ph0)**2)+c**2)
                self.v0 = self.v0 + (h*c/2.)*self.pv0/np.sqrt((np.linalg.norm(self.pv0)**2)+c**2)

                self.pW = mu*self.pW
                self.ph0 = mu*self.ph0
                self.pv0 = mu*self.pv0
...
if __name__ == '__main__':
    ...
    train_temps = [temps[i] for i in range(0, len(temps), config.rgd_skip)]
    N = config.N
    for i, T in enumerate(train_temps):
        ...
        # training RBM
        rbm = RBM(n_hidden=config.rgd_n_hidden,
                  batch_size=config.rgd_batch_size,
                  learning_rate=config.rgd_learning_rate,
                  mu=config.rgd_mu,
                  c=config.rgd_c,
                  n_iterations=config.rgd_iterations)
        rbm.fit(X)
        pickle.dump(rbm.training_errors,
                    open(config.rgd_conv_format%(T,N), 'wb'))
        ...
        pickle.dump(states, open(config.rgd_file_format%(T, N), 'wb'))
