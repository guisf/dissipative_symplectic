...
class RBM():
    ...
    def __init__(self, n_hidden=128, learning_rate=0.1, 
    ...
    def fit(self, X, y=None):
        ...
        for _ in self.progressbar(range(self.n_iterations)):
            ...
            for batch in batch_iterator(X, batch_size=self.batch_size):
                ...
                # gradient descent
                self.W = self.W - self.lr*g1
                self.h0 = self.h0 - self.lr*g2
                self.v0 = self.v0 - self.lr*g3
...
if __name__ == '__main__':
    ...
    train_temps = [temps[i] for i in range(0, len(temps), config.gd_skip)]
    N = config.N
    for i, T in enumerate(train_temps):
        ...
        Z[Z==-1] = 0 
        idx = np.random.choice(range(Z.shape[0]), config.gd_num_train_points,
                               replace=False)
        X = Z[idx]
        
        # training RBM
        rbm = RBM(n_hidden=config.gd_n_hidden,
                  batch_size=config.gd_batch_size,
                  learning_rate=config.gd_learning_rate,
                  n_iterations=config.gd_iterations)
        rbm.fit(X)
        pickle.dump(rbm.training_errors, 
                    open(config.gd_conv_format%(T,N), 'wb'))
        ...
        pickle.dump(states, open(config.gd_file_format%(T, N), 'wb'))
        print("step %i/%i ..."%(i+1, len(train_temps)))
        data = pickle.load(open(config.monte_carlo_output%(T, N), 'rb'))
